...
def port_path_exists(port_name: str) -> bool:
    if os.name == "nt":
...
class UpgradeSession:
    def __init__(self, port: str, image_path: Path, log_path: str, post_boot_seconds: float):
        ...
        # Preserve incoming CR/LF bytes as-is on Windows. Without this,
        # text-mode newline translation turns device "\r\n" into "\r\r\n".
        self.log_file = open(log_path, "w", encoding="utf-8", buffering=1, newline="")
    ...
    def reboot_from_app_prompt(self, state: str):
        flash_busy_marker = b"flash_is_busy, do not excute reboot"
        command_not_found_marker = b"Command not found!"
        reboot_payload = "\r\n\r\nreboot\r\n"
        reboot_label = "reboot"
        if state == "slave":
            reboot_payload = "app reboot 500\r\n"
            reboot_label = "app reboot 500"

        sent_d = False
        reboot_attempts = 0
        deadline = time.time() + 120
        tail = bytearray()
        fallback_to_plain_reboot = False

        while time.time() < deadline:
            reboot_attempts += 1
            current_payload = "\r\n\r\nreboot\r\n" if fallback_to_plain_reboot else reboot_payload
            current_label = "reboot" if fallback_to_plain_reboot else reboot_label
            self.record(
                f"[{now_ts()}] rebooting from @{state} prompt (attempt {reboot_attempts}) via {current_label}"
            )
            self.write_ascii(current_payload, current_label)
            ...
            while time.time() < attempt_deadline:
                ...
                if data:
                    ...
                    if (
                        state == "slave"
                        and not fallback_to_plain_reboot
                        and b"[root /]#" not in tail
                        and command_not_found_marker in tail
                    ):
                        self.record(
                            f"[{now_ts()}] app reboot command was not accepted; falling back to plain reboot"
                        )
                        fallback_to_plain_reboot = True
                        tail.clear()
                        break
                    if flash_busy_marker in tail:
                        self.record(f"[{now_ts()}] reboot blocked by flash busy; waiting 1s before retry")
                        tail.clear()
                        time.sleep(1)
                        break
        ...
        for baudrate in (460800, 115200):
            ...
            try:
                ...
                if state in ("master", "slave"):
                    self.reboot_from_app_prompt(state)
                    return
            finally:
                if state is None and self.ser is not None:
                    ...
                    self.ser = None
        raise UpgradeError(
            "unable to detect @master>>, @slave>>, [root /]#, or [config /]# at 460800/115200"
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--port",
        required=True,
        help="Serial device path, for example /dev/ttyUSB0 or COM11",
    )
...
def main():
    ...
    if not port_path_exists(port_path):
